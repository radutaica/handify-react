// Cache the serialize-error module import
let serializeErrorModule = null;
...
const reportErrorToRemote = async ({ error }) => {
  ...
  try {
    // Dynamically import serialize-error (ES module)
    if (!serializeErrorModule) {
      serializeErrorModule = await import('serialize-error');
    }
    const { serializeError } = serializeErrorModule;
